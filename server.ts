...
async function startServer() {
  ...
  app.get('/api/drive-image', async (req, res) => {
    try {
      ...
      // We try fetching the thumbnail because it is extremely lightweight, fast, and does not consume excessive bandwidth
      ...
      res.redirect(`https://drive.google.com/thumbnail?id=${req.query.id}&sz=w800`);
    }
  });
