...
const convertDriveImageUrl = (url: string): string => {
  ...
  if (driveId) {
    // 💡 ทางแก้ที่สมบูรณ์แบบสำหรับโดเมนจริง (Custom Domain) และปัญหาภาพแตก/ภาพเบลอ:
    // ดึงรูปตรงจาก Google Drive Thumbnail ในความละเอียดระดับ Ultra High Definition (sz=w1600)
    // โดยใช้คุณสมบัติ referrerPolicy="no-referrer" บนแท็ก <img> เพื่อข้ามการตรวจสอบ Referer Header
    // วิธีนี้ทำงานในเครื่องของผู้ใช้โดยตรง ไม่ใช้ Proxy Server ทำให้ความเร็วสูงสุด ไม่โดน 403 และไม่ติดคอขวด
...
// Help handle image error fallback beautifully
const handleImageError = (e: React.SyntheticEvent<HTMLImageElement, Event>) => {
  ...
  if (driveId) {
    if (attempts === 0) {
      // Fallback 1: Try loading direct lh3.googleusercontent.com/d/ URL with referrerPolicy
      img.src = `https://lh3.googleusercontent.com/d/${driveId}`;
    } else if (attempts === 1) {
      // Fallback 2: Try docs.google.com/uc endpoint directly
      img.src = `https://docs.google.com/uc?export=view&id=${driveId}`;
    } else if (attempts === 2) {
      // Fallback 3: Try our built-in backend proxy endpoint (if available)
